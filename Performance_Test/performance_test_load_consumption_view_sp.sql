CREATE OR REPLACE PROCEDURE MH_PERFORMANCE_TEST.MH_CONSUMPTION.SP_LOAD_CONSUMPTION()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS '
BEGIN
    ----------------------------------------------------------------
    -- 0. DROP (truncate-equivalent) EXISTING VIEWS
    ----------------------------------------------------------------
    DROP VIEW IF EXISTS V_COMPANY_METRICS;
    DROP VIEW IF EXISTS V_GLOBAL_METRICS;
    DROP VIEW IF EXISTS V_MASTER_ANALYTICS;

    ----------------------------------------------------------------
    -- 1. MASTER ANALYTICS VIEW
    ----------------------------------------------------------------
   create or replace view MH_PERFORMANCE_TEST.MH_CONSUMPTION.V_MASTER_ANALYTICS(
    -- (Column list remains exactly the same)
    OUTCOME_ID,
    SURVEY_ID,
    COMPANY_ID,
    IMPROVEMENT_SCORE,
    ...
    SELECT
        ...
        f.MEDICATION_ADHERENCE,
        f.SYMPTOM_SEVERITY,
        f.RECOMMEND_TREATMENT,
    ...
    FROM MH_PERFORMANCE_TEST.MH_HARMONIZED.FACT_TREATMENT_OUTCOMES AS f
    -- CHANGED TO LEFT JOIN HERE --
    LEFT JOIN MH_PERFORMANCE_TEST.MH_HARMONIZED.DIM_RESPONDENT AS r
        ON f.SURVEY_ID = r.SURVEY_ID
    -- CHANGED TO LEFT JOIN HERE --
    LEFT JOIN MH_PERFORMANCE_TEST.MH_HARMONIZED.DIM_COMPANY AS c
        ON f.COMPANY_ID = c.COMPANY_ID
    -- CHANGED TO LEFT JOIN HERE --
    LEFT JOIN MH_PERFORMANCE_TEST.MH_HARMONIZED.DIM_TREATMENT_HISTORY AS dth
        ON f.TREATMENTHISTORYKEY = dth.TREATMENT_HISTORY_KEY
    -- (These were already LEFT JOINs, so they are fine)
    LEFT JOIN MH_PERFORMANCE_TEST.MH_HARMONIZED.DIM_DATE AS d_followup
        ON f.FOLLOWUPDATEKEY = d_followup.DATEKEY
    LEFT JOIN MH_PERFORMANCE_TEST.MH_HARMONIZED.DIM_DATE AS d_diag
    ...
    ----------------------------------------------------------------
    CREATE OR REPLACE VIEW  MH_PERFORMANCE_TEST.MH_CONSUMPTION.V_COMPANY_METRICS AS
    SELECT
        COMPANY_NAME,
